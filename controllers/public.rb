  ...
  class Public < Kenji::Controller
    ...
    get '/about' do
      ...
      unless message = Cache.get('about')
        Cache.set('about', message = Curl.get(url), 60)
      end

      {
        status: 200,
        message: message.body_str.force_encoding('UTF-8'),
      }
    end
    ...
    get '/important-info' do
      ...
      unless message = Cache.get('important-info')
        Cache.set('important-info', message = Curl.get(url), 60)
      end
    ...
    get '/message' do
      ...
      unless message = Cache.get('message')
        Cache.set('message', message = Curl.get(url), 60)
      end
